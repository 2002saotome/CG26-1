...
//スケーリングを計算し、ワールド行列に乗算
	...
	matRot = XMMatrixIdentity(); //変形リセット
	matScale = XMMatrixScaling(scale.x, scale.y,scale.z);
	
	//XMMATRIX matRot;  //回転行列
	
	matRot *= XMMatrixRotationZ(rotation.z);  //Z軸まわりに45度回転
	matRot *= XMMatrixRotationX(rotation.x);  //X軸まわりに45度回転
	matRot *= XMMatrixRotationY(rotation.y);  //Y軸まわりに45度回転
...
//回転を計算し、ワールド行列に乗算
	...
	while (true) {
		...
		//matTrans=XMMatrixTransformation(position.x,po)
